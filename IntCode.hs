...
data State = State { sDone :: Bool,
                     sPos :: Int,
                     sRel :: Int,
                     sProg :: Array Int Int,
                     sInp :: [Int]} deriving (Show)
...
run :: State -> (State, [Int])
run s@(State cdone cpos crel cmem cinps) = if cdone then (s, [])
  else runST $
                ...
                Tri top ->
                  do p1 <- readMem m1 (p + 1)
                     ...
                     case top of
                       ...
                       LessT -> writeMem m3 p3 (if p1 < p2 then 1 else 0)
                       Equal -> writeMem m3 p3 (if p1 == p2 then 1 else 0)
...
addInps newInps st@(State {sInp = i}) = st {sInp = (i ++ newInps)}
...
loopUntilDone = chain' []
 where chain' (s:_)  inps [] | sDone s = last inps
       chain' states inps [] = chain' [] inps  $ reverse states
       chain' done   inps (s:sts) = chain' (ns:done) out sts
         where  (ns, out) = run (addInps inps s)
...
main = do let res = map (snd . run . flip initState []) tests
          ...
          print o1
          let (_, o2) = run $ initState input [2]
          print o2
